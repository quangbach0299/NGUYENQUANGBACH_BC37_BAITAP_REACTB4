...
export const BaiTapBookingTicket = (state = stateDefault, action) => {
  switch (action.type) {
    case "DAT_GHE": {
      const updatechairSelected = [...state.chairSelected];
      const foundIndex = updatechairSelected.findIndex(
        (item) => item.soGhe === action.gheDuocChon.soGhe
      );
      if (foundIndex === -1) {
        updatechairSelected.push(action.gheDuocChon);
      } else {
        updatechairSelected.splice(foundIndex, 1);
      }

      state.tongTien = updatechairSelected.reduce((total, item) => {
        return (total += item.gia);
      }, 0);

      return { ...state, chairSelected: updatechairSelected };
    }
    case "HUY_GHE": {
      const updatechairSelected = [...state.chairSelected];
      const foundIndex = updatechairSelected.findIndex(
        (item) => item.soGhe === action.removeChair
      );
      if (foundIndex !== -1) {
        updatechairSelected.splice(foundIndex, 1);
      }
    ...
    case "TINH_TIEN": {
      const updateDanhSachGhe = state.danhSachGhe.map((hang, index) => {
        hang.danhSachGhe.map((ghe, index) => {
          state.chairSelected.forEach((item) => {
      ...
      return { ...state, danhSachGhe: updateDanhSachGhe, chairSelected: [] };
    }
    default:
      return { ...state };
  }
